...
```
- Переключение на другого пользоваиеля и создание папки + файла в ней:
```
📌 1. Вариант: с sudo (если у тебя есть права администратора)
sudo -u имя_пользователя -i

После этой команды ты окажешься в shell пользователя john. Далее:
mkdir ~/my_new_folder

Чтобы выйти обратно в свою учётную запись:
exit

✅ 1. Создать пустой файл
touch /путь/к/папке/имя_файла
Пример:
touch ~/my_new_folder/example.txt
```
---
